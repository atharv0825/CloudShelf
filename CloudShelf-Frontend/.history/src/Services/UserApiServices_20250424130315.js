...
export const deleteImage = async (imageURL) => {
    try {
        ...
        console.log(email);
        const response = await fetch(`http://localhost:9090/cloudshelf/api/delete-image?email=${email}&imageUrl=${encodeURIComponent(imageURL)}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `${token}`
            }
...
xport const getFiles = async (email) => {
    try {
        const token = localStorage.getItem('token')
